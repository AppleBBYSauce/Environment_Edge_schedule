...
class Buffer(Dataset):
    ...
    def add_data(self, action, action_prob, state, reward):
        ...
        self.action.appendleft(action.squeeze(0))
        self.state.appendleft(state.squeeze(0))
        self.reward.appendleft(reward.squeeze(1))
        self.action_prob.appendleft(action_prob.squeeze(0))
    ...
    def BufferLoader(self, batch_size: int):
        return DataLoader(self, batch_size=batch_size, pin_memory=True, shuffle=True)
