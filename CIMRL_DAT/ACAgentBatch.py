...
class ACAgentBatch:
    ...
    def process_post(self, SW, TR, CT, cpu_usage_ratios, memory_usage_ratios, p_multiple):
        ...
        for agents in range(self.agent_nums):
            self.task_queue[agents].op(func=lambda x: x + self.refresh_frequency, key="rd")
            self.re_direct_queue[agents].op(func=lambda x: x + self.refresh_frequency, key="rd")
    ...
    def pre_process(self,
        ...
        mask_span_time = np.where(task_nums > 0, (self.cpu_cycles * task_nums) / (self.f * p_multiple),
                                  1 * self.tau * [self.arrive_ratio + np.array(
                                      [3, 0, 0]
                                  )]).sum(1)
