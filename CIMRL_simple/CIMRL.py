...
class Actor(torch.nn.Module):
    ...
    def __init__(self, hidden_nums: int, freedom_degree: list, state_nums: int, node_nums: int, drop_out=0.1):
        ...
        self.VAR = torch.nn.Parameter(torch.ones(size=(node_nums, self.action_nums, self.freedom_nums)) * 0.1,
                                      requires_grad=False)
    ...
    def forward(self, x, edge_index, shapley_index=None):
        ...
        # h_I = []
        # for idx, rnn in enumerate(self.bi_rnn):
        #     h_I.append(rnn(x[:, :, idx, :])[0])
        #
...
class ChIMRL(torch.nn.Module):
    ...
    def return_actor_update_parameters(self):
        return [{"params": self.global_gnn_update.parameters()}, {"params": self.actor_update.parameters()}]
