...
class Env:
    ...
    def __init__(self, service_nums: int, f: list[int], rho: list[float], tr: list[int], dm: list[int], gamma: float,
                 ...
                 local_edge, batch_size, soft_update_weight, soft_update_step, buffer_size: int, clip, sigma_gumbel=2,
    ...
    def update(self, step):
        ...
        if step > 25:
            ...
            for counter, data in enumerate(self.buffer.BufferLoader(self.batch_size)):
                ...
                states_t_plus = states[:, 1, :, :].to(cmp_device)
                with autocast():
                    action_t, action_probs_t, action_entropy, _ = self.nn.generate_act(observation=states_t,
                                                                                       update=True)
                    Q_t, y = self.nn.return_ChI_Q(states_t=states_t,
                                                  states_t_plus=states_t_plus,
                                                  reward=reward)
                    sample_important = torch.exp(action_probs_t - action_probs_t_sample)
                    TD_error = (y - Q_t).data
                    surr1 = sample_important * TD_error
                    surr2 = torch.clamp(sample_important, 1 - self.clip, 1 + self.clip) * TD_error
                    loss = torch.nanmean(torch.min(surr1, surr2) + 0.01 * action_entropy, dim=0).nansum()
...
if __name__ == '__main__':
    ...
    e = Env(service_nums=service_nums, f=f, rho=rho, tr=tr, dm=dm, gamma=gamma,
            ...
            buffer_size=buffer_size, clip=clip)
