...
class Env:
    ...
    def __init__(self, service_nums: int, f: list[int], rho: list[float], tr: list[int], dm: list[int], gamma: float,
        ...
        self.opt_critic = torch.optim.AdamW(params=self.nn.critic_update.parameters(), lr=self.lr_critic)
        self.opt_actor = torch.optim.AdamW(params=self.nn.actor_update.parameters(), lr=self.lr_actor)
    ...
    def step(self):
        ...
        if self.round_count - self.start_record > self.delta_t:
            ...
            self.update_round += 1
            print(
                f"load ratio std: {self.round_reward[:, 0].std()}, task latency mean: {self.round_reward[:, 1].sum()}, "
    ...
    def process(self, new_tasks: list[TaskQueue]):
        ...
        p_multiple, I_mig_multiple = actions_constrain
        self.Devices.p = p_multiple
        p_multiple, I_mig_multiple = p_multiple.numpy(), I_mig_multiple.numpy()
    ...
    def update(self, step):
        ...
        for counter, data in enumerate(self.buffer.BufferLoader(self.batch_size)):
            ...
            with autocast():
                Q_t, y = self.nn.return_mean_field_Q(states_t=states_t,
                                                     states_t_plus=states_t_plus,
                                                     reward=reward,
                                                     action_t=action_t,
                                                     action_probs_t=action_probs_t)
                loss = torch.abs(y - Q_t).mean(dim=0).sum()
        ...
        if step > 100:
            ...
            for counter, data in enumerate(self.buffer.BufferLoader(self.batch_size)):
                ...
                with autocast():
                    ...
                    # Q_t, _ = self.nn.get_pre(observation=states_t, actions=self.nn.return_avg_joint_actions(actions=action_t))
                    Q_t, y = self.nn.return_mean_field_Q(states_t=states_t,
                                                         states_t_plus=states_t_plus,
                                                         reward=reward,
                                                         action_t=action_t,
                                                         action_probs_t=action_probs_t)
                    loss = (y - Q_t).mean(dim=0).sum()
...
if __name__ == '__main__':
    ...
    e = Env(service_nums=service_nums, f=f, rho=rho, tr=tr, dm=dm, gamma=gamma,
            ...
            state_nums=12 * service_nums, freedom_nums=[service_nums for _ in range(2)], hidden_nums=hidden_nums,
