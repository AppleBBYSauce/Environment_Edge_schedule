...
class Actor(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        for idx, sigma in enumerate(self.decide_In_sigma):
            action_parameter_In_sigma.append(torch.clip(sigma(h), min=0))
        ...
        #     action_In_sigma)).data

        sample_action = (action_mu + torch.randn(size=action_mu.shape, device=action_mu.device) * self.VAR).data
    ...
    def constrain_action(self, task_nums, neighbor_nums, local_edges, actions):
        ...
        p = torch.softmax(actions[:, :, 1, :], dim=2)
        I_mig = []
        for idx, (neighbor_num, local_edge) in enumerate(zip(neighbor_nums, local_edges)):
            ...
            I_mig.append(local_edge.gather(1, ((neighbor_num - 1) * actions[:, idx, 2, :]).floor().long()))
        return p[0], I_loc[0], torch.stack(I_mig, dim=1)[0]
...
class MFAC(torch.nn.Module):
    ...
    def __init__(self, state_nums: int,
        ...
        self.critic = Critic(hidden_nums=hidden_nums, action_nums=sum(freedom_nums), output_nums=1,
                             node_nums=agents_nums, state_nums=state_nums)
    ...
    def return_mean_field_Q(self, states_t, states_t_plus, action_t, action_probs_t, reward):
        reward = (reward * self.gamma).sum(1)
        ...
        V_s_prime = self.Q_mul_prob(Qs=Q_t_plus.data, probs=torch.exp_(action_probs_t))
