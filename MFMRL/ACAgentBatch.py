...
class ACAgentBatch:
    ...
    def __init__(self, service_nums: int, f: list[int], f_mean: int, rho: list[float], tr: list[int], dm: list[int],
        ...
        self.tau = np.ceil((self.cpu_cycles*5) / (self.f / self.service_nums))
    ...
    def get_states(self):
        states = []
        for idx, tq in enumerate(self.task_queue):
            states.append(
                torch.concat([
    ...
    def process_post(self, SW, TR, CT, cpu_usage_ratios, memory_usage_ratios, p_multiple):
        ...
        P = ((self.f * p_multiple) / self.cpu_cycles) * np.stack(destructure_task_sizes, axis=0)
    ...
    def pre_process(self,
        ...
        for I_loc, temporary_state_inf in zip(I_loc_multiple, self.temporary_state_inf):
            drop_nums = temporary_state_inf.get_size() - I_loc
        ...
        SW = np.array([np.sum(re_direct_queue.return_sum(key="rd")) * self.sw for re_direct_queue in self.re_direct_queue])

        # calculate the transmission latency
        TR = np.array([np.sum(task_queue.return_sum(key="dl") / self.tr[idx]) for idx, task_queue in enumerate(self.task_queue)])

        # calculate the calculation latency
        arrive_ratio = I_loc_multiple / self.refresh_frequency
        ...
        sojourn_time = np.where(sojourn_time > 0, service_ratio, 2 / (1 * self.tau))
