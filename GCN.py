...
class GCNConv(torch.nn.Module):
    ...
    def __init__(self, input_num, out_num, dropout, node_nums,ab:bool=False):
        super().__init__()
...
class GCN(torch.nn.Module):
    ...
    def __init__(self, input_num, hidden_num, out_num, layer, dropout, edge_index, node_num):
        ...
        if layer == 1:
            self.GCNConvs = torch.nn.ModuleList([GCNConv(input_num=input_num, out_num=out_num, dropout=dropout, node_nums=node_num)])
        else:
            self.GCNConvs = torch.nn.ModuleList([GCNConv(input_num=input_num, out_num=hidden_num, ab=True, dropout=dropout, node_nums=node_num)] +
                                                [GCNConv(input_num=hidden_num, out_num=hidden_num, ab=True, dropout=dropout, node_nums=node_num) for _ in range(layer-2)] +
                                                [GCNConv(input_num=hidden_num, out_num=out_num, dropout=dropout, node_nums=node_num)])

    def forward(self, x, edge_index):
        for gnn in self.GCNConvs:
            x = gnn(x, edge_index, self.norm)
        return x